					...
					<div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 relative -top-36">
						{cardHome.map((item, i) => (
							<div key={i} className="relative">
								...
								<Link href={`/tech-guide/${item.category}`} className="absolute bottom-5 left-5 text-white text-base lg:text-xl uppercase font-semibold ">{item.text}</Link>
							</div>
						))}
					</div>
				</div>
